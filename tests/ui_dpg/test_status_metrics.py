...
def test_format_frame_badge_shows_fps_for_live_sources():
    assert format_frame_badge(SourceType.CAMERA, 40.0) == "Frame: 40.0 ms / 25.0 FPS"
    assert format_frame_badge(SourceType.SCREEN, 50.0) == "Frame: 50.0 ms / 20.0 FPS"


def test_format_frame_badge_hides_fps_for_still_sources():
    assert format_frame_badge(SourceType.IMAGE, 40.0) == "Frame: 40.0 ms"
    assert format_frame_badge(SourceType.NONE, 40.0) == "Frame: 40.0 ms"
